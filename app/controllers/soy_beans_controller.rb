...
class SoyBeansController < ApplicationController
  ...
  def csv_cleaner
    key_values_hash = { date: ['april', 'may', 'june', 'july', 'august', 'september', 'october', '?'],
                        ...
                        leafspots_marg: ["\tw-s-marg", 'no-w-s-marg', 'dna', '?'],
  ...
  def import(file)
    CSV.parse(File.read(file), headers: false) do |soy|
      data_dump = {
